...
// Conditional Global Declarations
  ...
  char SCALED_RH[8]; //For printing float RH var over snprintf()
  char SCALED_T[8]; //For printing float T var over snprintf()
  ...
  /*  PRINT TO SERIAL  */
    ...
    if (Serial2.available() != 0) {  // if Boron404 requests data...
      ...
      if (boronRequest == boronGoal) {
        ...
        Serial3.flush();
        delay(100);
        while(millis() - start < 250) { //250 ms timeout limit
          if (Serial3.available() != 0) {
            unoData = Serial3.readStringUntil('\n');
            unoData.trim();
            char unoDataCharA = unoData.charAt(0);
            if (unoDataCharA == 'A'){
              unoData = unoData.substring(1);
            } else {
              unoData = ",,";
            }
            break;
