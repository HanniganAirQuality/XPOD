...
// Conditional Global Declarations
  ...
  String unoData; //unoData
  char SCALED_RH[10]; //For printing float RH var over snprintf()
  char SCALED_T[10]; //For printing float T var over snprintf()
  String boronGoal = "Boron requesting data"; //Message we're waiting to hear to trigger the document 
#endif //CROSSTALK_ON
...
void setup() {
  ...
  #if CROSSTALK_ON
    Serial2.begin(9600); //To Boron 
    Serial3.begin(9600); //To UNO
    // unoData = "404,404,404";
  #endif //CROSSTALK_ON
...
void loop() {
  ...
  #endif //SERIAL_ENABLED
  #if CROSSTALK_ON
    if (Serial2.available() != 0) // if Boron404 requests data...
    {
      String boronRequest = Serial2.readStringUntil('\n'); //Reading incoming string up till \n from Serial2
      boronRequest.trim(); //Trims extra characters 
      ...
      if (boronRequest == boronGoal) {
        ...
        {
          unoData = Serial3.readStringUntil('\n');
          unoData.trim();
          Serial.print(F("\n"));
          Serial.print(unoData);
        }
        else
        {
          unoData = ",,";
        ...
        int16_t QS1_C1 = quadstat_data.QS1_C1;
        int16_t QS1_C2 = quadstat_data.QS1_C2;
        int16_t QS2_C1 = quadstat_data.QS2_C1;
        int16_t QS2_C2 = quadstat_data.QS2_C2;
        int16_t QS3_C1 = quadstat_data.QS3_C1;
        int16_t QS3_C2 = quadstat_data.QS3_C2;
        int16_t QS4_C1 = quadstat_data.QS4_C1;
        int16_t QS4_C2 = quadstat_data.QS4_C2;
        float RH = bme_data.RH;
        dtostrf(RH, 3, 2, SCALED_RH);
        float T = bme_data.T;
        dtostrf(T, 3, 2, SCALED_T);
        uint16_t CO2 = CO2_module.getS300CO2();
        int16_t CO_main = ads_data.Worker;
        int16_t CO_Aux = ads_data.Auxiliary;
        delay(100);
        char buffer[256];
        sprintf(buffer, "%u,%u,%u,%d,%d,%d,%d,%d,%d,%d,%d,%s,%s,%u,%d,%d,%s", h, m, s, QS1_C1, QS1_C2, QS2_C1, QS2_C2, QS3_C1, QS3_C2, QS4_C1, QS4_C2, SCALED_RH, SCALED_T, CO2, CO_main, CO_Aux, unoData.c_str());
