 ...
 * @author  Percy Smith
 * @date 	  July 28, 2025 
 * @log     Trying to simplify code to make it easier to connect to cellular
 * 
 * @errors  Add a timeout timer so that at like 30 s no response, it timesout
 ******************************************************************************/
#include "xpod_node.h"
...
void loop() {
  ...
  //poll for plantower data first cause it has to run a while loop and i'm paranoid !!
    bool pm_available = 0;
    while(!pm_available)   { //add a timer that can timeout
      pm_available = pms5003.readPMSdata(&Serial1);
      delay(100);
      if(pm_available) { pms_data = pms5003.returnPMdataset(); }
    } //while(!pm_available)
    delay(100);
  #endif //PMS_ENABLED
