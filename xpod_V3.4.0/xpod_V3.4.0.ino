 ...
 *
 * @file    xpod_V3.4.0.ino
 * @version Percy's 3.4.0
 * @brief   Timestamps UTC
 *
 * @author  Percy Smith
 * @date 	  May 13, 2026 
 * @log     Optimizing characters for XPOD6M2 functionality
 *          Updated libraries as of 07/28/2025
 ******************************************************************************/
#include "xpod_node.h"
...
#if CONFIGA
  char fileName[] = "YPODID_MM_DD.TXT";
#else
...
void setup() {
  ...
  DateTime now = rtc.now();     //pulls setup() time so we have one file name per run in a day
    ...
    D = now.day();
  #if CONFIGA
    //ADDED TXT SUPPORT (EVIL)
    sprintf(fileName, "%s_%02u_%02u.TXT", xpodID, M, D);    //char array for fileName
  #else
